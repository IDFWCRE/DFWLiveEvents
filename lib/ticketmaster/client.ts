...
async function fetchDiscoveryPage(
  ...
  options.log?.(`[ticketmaster] Requesting ${city} / ${classificationName} / page ${page}`);
  const response = await fetchWithTimeout(url, timeoutMs);

  if (response.status === 429) {
    options.log?.(`[ticketmaster] Rate limited for ${city} / ${classificationName} / page ${page}; retrying once`);
    await sleep(1200);
    const retry = await fetchWithTimeout(url, timeoutMs);
    if (!retry.ok) {
      throw new Error(`Ticketmaster retry failed for ${city}/${classificationName}/page ${page}: ${retry.status}`);
    }
    const retryJson = (await retry.json()) as DiscoveryResponse;
    options.log?.(
      `[ticketmaster] Received ${retryJson._embedded?.events?.length || 0} events for ${city} / ${classificationName} / page ${page}`
    );
    return retryJson;
  }

  if (!response.ok) {
    throw new Error(`Ticketmaster request failed for ${city}/${classificationName}/page ${page}: ${response.status}`);
  }

  const json = (await response.json()) as DiscoveryResponse;
  options.log?.(
    `[ticketmaster] Received ${json._embedded?.events?.length || 0} events for ${city} / ${classificationName} / page ${page}`
  );
  return json;
}
...
export async function fetchTicketmasterEvents(options: TicketmasterFetchOptions = {}): Promise<TicketmasterFetchSummary> {
  ...
  for (const city of ticketmasterCities) {
    for (const classificationName of classifications) {
      try {
        options.log?.(`[ticketmaster] Starting ${city} / ${classificationName}`);
        const firstPage = await fetchDiscoveryPage(city, classificationName, 0, options);
        events.push(...(firstPage._embedded?.events || []));

        const totalPages = Math.min(firstPage.page?.totalPages || 1, maxPagesPerQuery);
        options.log?.(`[ticketmaster] ${city} / ${classificationName}: fetching ${totalPages} page(s)`);

        for (let page = 1; page < totalPages; page += 1) {
          await sleep(275);
          const nextPage = await fetchDiscoveryPage(city, classificationName, page, options);
          events.push(...(nextPage._embedded?.events || []));
        }

        options.log?.(`[ticketmaster] Finished ${city} / ${classificationName}`);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        errors.push(message);
        options.log?.(`[ticketmaster] Error for ${city} / ${classificationName}: ${message}`);
      }
